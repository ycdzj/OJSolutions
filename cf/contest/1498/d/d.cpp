    ...
    void solve() {
        ...
        for(int i = 1; i <= n; i++) {
            ...
            for(long long j = 0; j < m; j++) {
                ...
                long long nxt_j;
                if(t == 1) {
                    nxt_j = (j * FRAC + x + FRAC - 1) / FRAC;
                } else {
                    nxt_j = (j * x + FRAC - 1) / FRAC;
                }
                if(nxt_j > m) {
                    continue;
                }
                ...
                nxt_item.op_cnt = 1;
                if(items[j].op_idx == i) {
                    nxt_item.op_cnt += items[j].op_cnt;
                }
                if(nxt_item.op_cnt > y) {
                    continue;
                }

                if(!items[nxt_j].vis) {
                    items[nxt_j] = nxt_item;
                } else if(items[nxt_j].op_idx == i) {
                    items[nxt_j].op_cnt = min(items[nxt_j].op_cnt, nxt_item.op_cnt);
                }
            }
        }
        vector<int> ans;
