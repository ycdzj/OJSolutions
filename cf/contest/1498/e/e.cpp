...
public:
    ...
    void solve() {
        vector<pair<int, int>> pr_list;
        for(int i = 0; i < n; i++) {
            for(int j = i + 1; j < n; j++) {
                pr_list.emplace_back(i, j);
            }
        }
        sort(pr_list.begin(), pr_list.end(), [&](pair<int, int> x, pair<int, int> y)->bool{
            return get_dis(x.first, x.second) > get_dis(y.first, y.second);
        });
        for(auto pr : pr_list) {
            int x = pr.first;
            int y = pr.second;
