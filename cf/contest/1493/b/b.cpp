...
class Solver {
    ...
    bool swap_digit(int val, int &res) {
        res = 0;
        for(int i = 0; i < 2; i++) {
            int cur_digit = val % 10;
            val /= 10;

            if(mirror[cur_digit] == -1) {
                return false;
            }

            res = res * 10 + mirror[cur_digit];
        }
        return true;
    }
    bool check(int timestamp) {
        int cur_m = timestamp % m;
        int cur_h = (timestamp / m) % h;

        int mirror_m;
        if(!swap_digit(cur_h, mirror_m)) {
            return false;
        }

        int mirror_h;
        if(!swap_digit(cur_m, mirror_h)) {
            return false;
        }

        return mirror_m < m && mirror_h < h;
    }
public:
    void read() {
        Io::read(h);
