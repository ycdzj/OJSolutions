...
public:
    ...
    vector<int> get_ans_from_left() {
        vector<int> ans;
        for(int i = k + 1; i <= n; i++) {
            ans.push_back(i);
        }
        for(int i = 1, sum = 0; i <= k - 1; i++) {
            sum += i;
            if(sum >= k) {
                break;
            }
    ...
    vector<int> get_ans_from_right() {
        vector<int> ans;
        for(int i = k + 1; i <= n; i++) {
            ans.push_back(i);
        }
        for(int i = k - 1; i >= 1 && i * 2 >= k; i--) {
            ans.push_back(i);
        }
        return ans;
    }
    void solve() {
        vector<int> ans1 = get_ans_from_left(), ans2 = get_ans_from_right(), ans;
