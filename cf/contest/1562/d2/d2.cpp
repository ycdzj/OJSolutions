...
class Solver {
    ...
    vector<int> get_ans(int l, int r) {
        int cur_val = sum[r] - sum[l - 1];
        ...
        if((r - l + 1) & 1) {
            ...
            int idx = *lower_bound(indices[pre_val + n].begin(), indices[pre_val + n].end(), l);

            return {idx};
        }

        vector<int> ans = get_ans(l, r - 1);
        ans.push_back(r);
        return ans;
    }
public:
