...
class Solver {
    ...
    int get_ans(const vector<int> &cur_a, const vector<int> &cur_b) {
        vector<int> c(cur_b.size() + 1);
        for(int i = 0, j = 0; i < cur_a.size() && j < cur_b.size(); ) {
            if(cur_a[i] == cur_b[j]) {
                c[j] = 1;
                i++;
                j++;
            } else if(cur_a[i] < cur_b[j]) {
                i++;
            } else {
                j++;
            }
        ...
        int ans = 0;
        for(int j = 0, cnt = 0, i = 0; j < cur_b.size(); j++) {
            while(cnt < cur_a.size() && cur_a[cnt] <= cur_b[j]) {
                cnt++;
            }
            while(i < cur_b.size() && cur_b[i] <= cur_b[j] - cnt) {
                i++;
            }
            ans = max(ans, j - i + 1 + c[j + 1]);
        }
        return ans;
    }
public:
