...
public:
    ...
    void solve() {
        set<pair<int, int>> same_set, diff_set;
        ...
        for(int i = 0; i < m; i++) {
            ...
            if(op == "+") {
                ...
                adj[{u, v}] = c[0];
                if(adj[{v, u}] == adj[{u, v}]) {
                    same_set.insert({min(u, v), max(u, v)});
                } else if(adj[{v, u}] != 0) {
                    diff_set.insert({min(u, v), max(u, v)});
                }
            } else if(op == "-") {
                int u, v;
                Io::read(u);
                Io::read(v);
                if(adj[{v, u}] == adj[{u, v}]) {
                    same_set.erase({min(u, v), max(u, v)});
                } else if(adj[{v, u}] != 0) {
                    diff_set.erase({min(u, v), max(u, v)});
                }
                adj[{u, v}] = 0;
            } else if(op == "?") {
                int k;
                ...
                bool ans;
                if(k & 1) {
                    ans = (!same_set.empty() || !diff_set.empty());
                } else {
                    ans = !same_set.empty();
                }
                if(ans) {
