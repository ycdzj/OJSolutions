...
public:
    ...
    void solve() {
        ...
        for(int i = a.length(); i >= 0; i--) {
            f[i % 2].resize(b.length() + 1);
            for(int j = b.length(); j >= 0; j--) {
                f[i % 2][j].assign(26, 0);
                for(int x = 0; x < 26; x++) {
                    f[i % 2][j][x] = 1;
                    if(i < a.length() && a[i] - 'a' != x) {
                        f[i % 2][j][x] = mod.add(f[i % 2][j][x], f[(i + 1) % 2][j][a[i] - 'a']);
                    }
                    if(j < b.length() && b[j] - 'a' != x) {
                        f[i % 2][j][x] = mod.add(f[i % 2][j][x], f[i % 2][j + 1][b[j] - 'a']);
                    }
                }
                if(i > 0) {
                    ans = mod.add(ans, f[i % 2][j][a[i - 1] - 'a']);
                }
                if(j > 0) {
                    ans = mod.add(ans, f[i % 2][j][b[j - 1] - 'a']);
                }
            }
        }
