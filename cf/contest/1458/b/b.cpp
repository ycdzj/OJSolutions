...
public:
    ...
    void solve() {
        ...
        for(int i = 1; i <= n; i++) {
            for(int j = 0; j <= n; j++) {
                for(int k = 0; k <= 10000; k++) {
                    ...
                    if(j > 0 && a[i - 1] <= k) {
                        if(f[(i - 1) & 1][j - 1][k - a[i - 1]] != -1) {
                            f[i & 1][j][k] = max(f[i & 1][j][k], f[(i - 1) & 1][j - 1][k - a[i - 1]] + b[i - 1]);
                        }
                    }
                }
            }
        ...
        for(int j = 1; j <= n; j++) {
            ...
            for(int k = 0; k <= 10000; k++) {
                if(f[n & 1][j][k] != -1) {
                    double cur_ans = min((double)k, f[n & 1][j][k] + (double)(sum_b - f[n & 1][j][k]) / 2);
                    ans = max(ans, cur_ans);
                }
            }
            printf("%.7f\n", ans);
