...
public:
    ...
    void solve() {
        ...
        long long ans = 0, total_diff_days = 0;
        vector<long long> sum(m), cnt(m);
        vector<bool> vis(m);
        bool flag = false;
        for(int j = 0, pre = 0; ; ) {
            if(total_diff_days + weight_of_b[j] >= k) {
                for(int i = 0; i < n; i++) {
                    if(total_diff_days >= k) {
                        break;
                    }
                    ans++;
                    if(a[i] != b[(j + i) % m]) {
            ...
            if(!vis[j] || flag) {
                ans += n;
                total_diff_days += weight_of_b[j];

                sum[j] = sum[pre] + weight_of_b[j];
                cnt[j] = cnt[pre] + 1;
                ...
                long long cur_sum = sum[pre] - sum[j] + weight_of_b[j];
                long long cur_cnt = cnt[pre] - cnt[j] + 1;

                long long cnt_round = (k - total_diff_days - 1) / cur_sum;
                ans += cnt_round * cur_cnt * n;
                total_diff_days += cnt_round * cur_sum;
