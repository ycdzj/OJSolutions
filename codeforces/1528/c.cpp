...
class Solver {
    ...
    void build_dfs_order(int u, int &cur) {
        dfs_order[u].first = ++cur;
        for(int v : adj_b[u]) {
            build_dfs_order(v, cur);
        }
        dfs_order[u].second = ++cur;
    }
    ...
    void add(int u) {
        cnt++;
        int l = dfs_order[u].first;
        int r = dfs_order[u].second;
        bit1.update(l, 1);
        bit2.update(l, u);
        bit2.update(r + 1, -u);
    }
