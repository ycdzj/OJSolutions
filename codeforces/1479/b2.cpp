...
public:
    ...
    void solve() {
        ...
        for(int i = 0; i < a.size(); i++) {
            ...
            int val = a[i];
            if(last_val[0] == val || last_val[1] == val) {
                continue;
            }

            if(last_val[0] == -1) {
                last_val[0] = val;
            } else if(last_val[1] == -1) {
                last_val[1] = val;
            } else if(indices[last_val[0]].empty()) {
                last_val[0] = val;
            } else if(indices[last_val[1]].empty()) {
                last_val[1] = val;
            } else if(indices[last_val[0]].front() < indices[last_val[1]].front()) {
                last_val[1] = val;
            } else {
                last_val[0] = val;
            }
